...
```bash
docker network create --driver bridge lrcs
```
...
#### 📂 Backend Repository Identification
🔗 **Repository**: `https://github.com/jas-nurturing-stage/LRCS_backend`
...
```bash
git clone https://github.com/jas-nurturing-stage/LRCS_backend
cd lrcs-backend
npm install
npm start
```
#### 📂 Frontend Repository Identification
🔗 **Repository**: `https://github.com/jas-nurturing-stage/LRCS_frontend`
...
```bash
git clone https://github.com/jas-nurturing-stage/LRCS_frontend
cd lrcs-frontend
npm install
npm run dev
```
...
📁 Create a `Dockerfile` inside the `lrcs-backend` directory:
```Dockerfile
...
```bash
docker image build --no-cache -t docjas143/lrcs-backend:latest .
```
![learner insights backend build](images/learner-insights-backend/build.png)

Push the image to Docker Hub.
```bash
docker image push docjas/lrcs-backend:latest
```

Run the container:

```bash
docker container run -d -p 3001:3001 --name lrcs-backend -e ATLAS_URI=mongodb://root:root@192.168.31.100:27017/lrcs?authSource=admin -e HASH_KEY=thisIsMyHashKey -e JWT_SECRET_KEY=thisIsMyJwtSecretKey -e PORT=3001 --network lrcs docjas143/lrcs-backend
```
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: lrcs-backend
        image: docjas143/lrcs-backend:latest
...
🔍 **Verify Pods**
```bash
kubectl get pods -n lrcs
```
📜 **View Logs to Confirm Communication**
```
kubectl logs deploy/backend -n lrcs
```
OR
```
kubectl logs pod/<pod_name> -n lrcs
```
...
Run a shell inside the user pod:
```
kubectl exec -it deploy/backend -n lrcs -- sh
```
From inside the pod, test communication:
```
curl http://backend.lrcs.svc.cluster.local:3001/health
```
...
```bash
kubectl port-forward service/backend -n=lrcs 3001:3001 --address=0.0.0.0
```
...
📁 Create a `Dockerfile` inside the `lrcs-frontend` directory:
```Dockerfile
# STAGE 1: Build
FROM node:22-alpine AS builder
...
Build the Docker image:
```bash
docker image build --no-cache --build-arg REACT_APP_API_BASE_URL=http://192.168.31.100:3001 -t docjas143/lrcs-frontend .
```
![learner insights frontend build](images/learner-insights-frontend/build.png)

Push the image to Docker Hub.
```bash
docker image push docjas143/lrcs-frontend:latest
```

Run the container:

```bash
docker container run -d -p 3000:80 --name lrcs-frontend --network lrcs docjas143/lrcs-frontend
```
...
📜 **View Logs to Confirm Communication**
```
kubectl logs deploy/frontend -n lrcs
```
OR
```
kubectl logs pod/<pod_name> -n lrcs
...
Run a shell inside the user pod:
```
kubectl exec -it deploy/frontend -n lrcs -- sh
```
From inside the pod, test communication:
```
curl http://frontend.lrcs.svc.cluster.local:3000
```
![learner insights frontend SVC](images/learner-insights-frontend/text-communication-in-pod.png)
#### 🧪 4 Test with Port Forwarding
```bash
kubectl port-forward service/frontend -n=lrcs 3000:3000 --address=0.0.0.0
```
🌍 Access the service in your browser or tool like Postman:
run it
```bash
...
🧪 Test Chart:
```
helm upgrade --install lrcs-backend helm/backend -f helm/backend/values.yaml --namespace lrcs --create-namespace
```
...
🧪 Test Chart:
```
helm upgrade --install lrcs-frontend helm/frontend -f helm/frontend/values.yaml --namespace lrcs --create-namespace
```
...
1.3. **Configure Pipeline**
  - In the pipeline configuration:
    ...
    - **SCM:** Git.
    - **Repository URL**: URL of the backend repository (e.g., https://github.com/jas-nurturing-stage/LRCS_backend.git).
...
2.1. **Create a New Pipeline**
  - Go to `Jenkins Dashboard > New Item`.
  - Enter a name (e.g., `lrcs-frontend`).
  - Select **Pipeline** and click **OK**.
...
2.3. **Configure Pipeline**
  - In the pipeline configuration:
      - **Definition:** Select `Pipeline script from SCM`
      - **SCM:** Git.
      - **Repository URL**: URL of the frontend repository (e.g., https://github.com/jas-nurturing-stage/LRCS_frontend.git).
      - **Credentials**: Add or Select Git credentials if required.
      - **Script Path**: Jenkinsfile.
      - **Branches to build**: `*/main`
...
3.3. **Configure Pipeline**
  - In the pipeline configuration:
      - **Definition:** Select `Pipeline script from SCM`
      - **SCM:** Git.
      - **Repository URL**: URL of the frontend repository (e.g., https://github.com/jas-nurturing-stage/ContainerOrchestration.git).
      - **Credentials**: Add or Select Git credentials if required.
      - **Script Path**: Jenkinsfile.
      - **Branches to build**: `*/main`
3. **Script Path**: `Jenkinsfile`
...
```Jenkinsfile
@Library('Shared') _
def config = securityConfig("docjas143/lrcs-backend:${BUILD_NUMBER}",'')
...
```Jenkinsfile
@Library('Shared') _
def config = securityConfig("docjas143/lrcs-frontend:${BUILD_NUMBER}",'')
        ...
        stage('Deploy with Helm') {
            steps {
                script {
                    ...
                    sh """
                        helm upgrade --install lrcs-${serviceName} ${chartPath} \
                        -f ${chartPath}/values.yaml \
                        --set image.repository=docjas143/lrcs-${serviceName} \
...
- **Pod CrashLoopBackOff**:
    ...
    - Run: kubectl describe pod <pod-name> -n lrcs.
